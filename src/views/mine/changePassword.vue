<template>
<div class=''>
    <van-nav-bar fixed placeholder title="修改登录密码" left-arrow @click-left="onClickLeft" />
    <div class="content">
        <div>
            <van-form>
                ...
                <van-field v-model="vailcode" name="vailcode" autocomplete="off" type="number" label="" placeholder="请输入6位验证码">
                ...
                <div style="">
                    <van-button class='mgt25' round block type="info" :loading="codeBtnLoading" :disabled="!isLogin" @click="onSubmit">确定</van-button>
                </div>
            </van-form>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        toRegister() {
            ...
            operateUserPassWd(params).then((res)=> {
                if(res.flag){
                    this.$toast(res.msg)
    ...
    //生命周期 - 挂载完成（可以访问DOM元素）
    mounted() {
        this.mobile = this.getUserInfo.phoneNo
    },
