...
<div style="background: #f8f8f8; height: 100vh">
    <van-nav-bar title="诊断记录" :fixed="true" :placeholder="true" :z-index="99" left-text="" left-arrow @click-left="onClickLeft" />
        ...
        <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" >
            <div class="diagnosticList" v-for="(value, index) in diagnosticList" :key="index" @click="jumpPage(value)" v-show="diagnosticList.length > 0">
                <div class="diagnosticList-left">
                    <div class="diagnosticList_img">
                        <img v-lazy="value.url" :key="value.url" alt="" />
                ...
                <div class="diagnosticList-right">
                    <div class="carryOn" v-if="value.isFinish != 'S03'">继续诊断</div>
                    <van-icon name="arrow" />
                </div>
            </div>
        </van-list>
    </van-pull-refresh>
    <NoResult class="all-center"  v-if="diagnostiShow"></NoResult>
            ...
            return new Promise((resolve, reject) => {
                ...
                getHuiXueTaskList(fonmData).then((res) => {
                    if (res.flag) {
                        if (!res.data) {
                            this.finished = true;
                            this.diagnostiShow = true
                            this.$toast('暂无数据');
                            return;
                        }else {
                            this.diagnostiShow = false
                        }
            ...
            return new Promise((resolve, reject) => {
                ...
                getHuiXueTaskList(fonmData).then((res) => {
                    if (res.flag) {
                        ...
                        this.this.diagnosticList = datrasx(this.this.diagnosticList);
