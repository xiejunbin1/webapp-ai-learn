<template>
<div class='' style="min-height:100vh;background:#E7F2FF">
    <van-nav-bar fixed placeholder title="诊断报告" left-text="">
        <template #left>
    ...
    <van-pull-refresh v-if="reportData && Object.keys(reportData).length>0" v-model="isLoading" @refresh="onRefresh">
        <div class='pd15'>
            ...
            <div class='base-info'>
                <div class='mgb20'>
                    <svg-icon class="svg" icon-class="icon-judge-report-base"></svg-icon>
                    <span class='name'>基本信息</span>
                </div>
                <div class='mgt20'>
                    <span class="label-name">诊断科目：</span>
                    <span class='label-input'>{{reportData.subjectTypeName}}</span>
                </div>
                <div class='mgt20'>
                    <span class="label-name">诊断章节：</span>
                ...
                <div class='mgt20'>
                    <span class="label-name">诊断时间：</span>
            ...
            <div class='rank-box'>
                ...
                <div class='title'>
                    <svg-icon class="svg" icon-class="icon-judge-report-rank"></svg-icon>
                    <span class='name'>诊断结果等级评定</span>
                </div>
            ...
            <div class='base-info' v-if="suggestType=='T01'">
                ...
                <van-tabs v-model="active">
                    ...
                    <van-tab title="巩固练习" v-if="suggestListT02.length>0">
                        <div class='learn-box' v-show="item.objectType=='T02'" @click="goToPractice(item)" v-for="(item,index) in suggestListT02.slice(0,suggestLength)" :key="index">
                            <div class="title-box">
                                {{item.sysCourseName}}
                            </div>
            ...
            <div class='base-info' v-if="suggestType=='T03'">
                ...
                <div>
                    <div class='paper-box' @click="goToJudge(item)" v-for="(item,index) in suggestList.slice(0,suggestLength)" :key="index">
                        <div class='img-box'>
                        ...
                        <div class="title-box">
                            ...
                            <div class='degree'>
                                难度：
                                <van-rate color="#FFDD88" model="item.suggestDegree=='D01'?2:item.suggestDegree=='D02'?3:4" void-icon="star" void-color="#D8D8D8" />
                            </div>
                        </div>
                    </div>
                    <div>
            ...
            <div class='base-info' v-if="suggestType=='T04'">
                ...
                <div>
                    ...
                    <div class='video-box' v-if="suggestList.length>0" @click="goToVideo(item)" v-for="(item,index) in suggestList.slice(0,suggestLength)" :key="index">
                        <div class='img-box'>
                            <img :src="item.objectIcon" class="object-icon" alt="">
...
export default {
    ...
    data() {
        return {
            query: {},
            scoreUrl: require('@/assets/img/home/score-background.png'),
            reportData: {
                pointViewList: [],
                score: 0
            },
            rankUrl: require('@/assets/img/home/rank-background.png'),
    ...
    methods: {
        ...
        goToVideo(item) {
            if (item.sysCourseId) {
                ...
                let classGrade = this.$options.filters.getGradeName(this.reportData.classGrade)
                let termType = this.reportData.termType == 'T01' ? '上册' : '下册'
                let titleName = `${this.reportData.textBookName}${classGrade}${this.reportData.subjectTypeName}${termType}`
